...
public class AdminAppointmentController {
    ...
    public void addAppointment(ActionEvent event) {
        ...
        """;
            try (PreparedStatement stmt = connection.prepareStatement(addAppointmentQuery)) {
                ...
                int doctorId = getDoctorIdByNameAndSpecialty(selectedDoctor , selectedSpecialty , connection);
    ...
    public void deleteAppointment() {
        ...
        confirmAlert.setContentText("Are you sure you want to delete the appointment with ID: " + selectedAppointment.getAppointmentId() + "?");
        ...
        confirmAlert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    ...
                    UPDATE schedules
                    SET remain_limit = remain_limit + 1
                    WHERE schedule_id = (
                ...
                """;
                    PreparedStatement updateStmt = con.prepareStatement(updateScheduleQuery);
                    updateStmt.setInt(1, selectedAppointment.getAppointmentId());
                    ...
                    successAlert.setContentText("Appointment deleted successfully.");
                    ...
                    errorAlert.setContentText("Failed to delete appointment.");
    ...
    private int getDoctorIdByNameAndSpecialty(String doctorName, String specialty, Connection connection) throws SQLException {
        String query = """
        SELECT d.doctor_id 
        FROM doctors d
        JOIN users u ON d.user_id = u.user_id
    ...
    """;
        ...
        throw new SQLException("Doctor not found for the given name and specialty.");
    }
    ...
    public void editAppointment(ActionEvent event) {
        ...
        List<String> statusOptions = List.of("Pending", "Completed");
        ChoiceDialog<String> dialog = new ChoiceDialog<>(selectedAppointment.getStatus(), statusOptions);
        dialog.setTitle("Edit Appointment Status");
        dialog.setHeaderText("Edit Status for Appointment ID: " + selectedAppointment.getAppointmentId());
        dialog.setContentText("Choose the new status:");

        String newStatus = dialog.showAndWait().orElse(null);

        if (newStatus == null || newStatus.equals(selectedAppointment.getStatus())) {
            return;
        }
        ...
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            stmt.setInt(2, selectedAppointment.getAppointmentId());
            stmt.executeUpdate();

            selectedAppointment.statusProperty().set(newStatus);

            Alert successAlert = new Alert(Alert.AlertType.INFORMATION);
            successAlert.setTitle("Success");
            successAlert.setHeaderText(null);
            successAlert.setContentText("Appointment status updated successfully.");
            successAlert.showAndWait();
            ...
            errorAlert.setTitle("Error");
            errorAlert.setHeaderText(null);
            errorAlert.setContentText("Failed to update appointment status: " + e.getMessage());
            errorAlert.showAndWait();
        }
    }
}